...
# BUG 2 수정: A 평점 4.0 → 4.5 (명세: A=4.5)
...
def average_score(student):
    """학생의 평균."""
    # BUG 1 수정: len(student) → 3 (딕셔너리엔 '이름' 포함 키 4개라서 /4가 됨)
...
def subject_average(students, subject):
    ...
    total = 0
    for stu in students:
        # BUG 3 수정: stu["국어"] → stu[subject] (subject 인수 무시하고 항상 국어만 합산)
        total += stu[subject]
...
def subject_top(students, subject):
    ...
    top = 0
    for stu in students:
        # BUG 4 수정: stu[subject] < top → > top (부등호 반대라 최솟값을 찾고 있었음)
        if stu[subject] > top:
            top = stu[subject]
    return top
...
def rank_list(students):
    """총점 기준 정렬."""
    # BUG 5 수정: reverse=True 추가 (총점 높은 학생이 1등 = 내림차순)
